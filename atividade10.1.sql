...
-- a)
create or replace view vw_vendas_produtos_qtd as
select iv.num_venda as venda,
	   p.nome as produto,
	   iv.quantidade as quantidade
from produto p
join itens_venda iv on p.cod_produto = iv.produto;

select produto, quantidade from vw_vendas_produtos_qtd;
...
create or replace view vw_vendedor_comprador as
select vendas.num_venda as venda,
	   vendedor.nome as vendedor,
	   cliente.nome as comprador,
	   vendas.data as data
from vendas
join vendedor on vendedor.id_vendedor = vendas.vendedor
join cliente on cliente.id_cliente = vendas.comprador;

select * from vw_vendedor_comprador;


-- d)

create or replace view vw_vendedor_qtd_vendas as
select vendedor.nome as vendedor,
	   COUNT(vendas.vendedor) as `Quantidade de vendas`
from vendedor
join vendas on vendas.vendedor = vendedor.id_vendedor
group by vendedor.nome;

select * from vw_vendedor_qtd_vendas;
